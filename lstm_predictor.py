...
def main():
    ...
    # Sequence
    # For each i, X = sequence of SEQ_LEN (scaled), y = [future_high, future_low] of (SEQ_LEN + PREDICT_AHEAD - 1)-th bar after i (use original prices)
    X, y = [], []
    ...
    def run_one_turn(turn_idx, timestamp, plot_first=None, plot_last=None):
        ...
        # true and predicted are now direct high/low prices
        ...
        if WRITE_CSV:
            ...
            plt.legend()
            plt.title(f"{symbol} LSTM Predicted vs True High/Low (direct price target, {timeframe} timeframe)")
            plt.xlabel("Datetime")
            plt.ylabel("Price")
